...
| Field | Meaning |
| --- | --- |
| `comparison_question` | Human-readable question tested by the selected comparison type |
| `observed_table` | Actual 2x2 count table used by the test |
...
| `relative_risk` | Ratio between group A share and group B share |
| `odds_ratio` | Ratio between the odds in group A and group B |
| `evidence_level` | `none`, `weak`, `moderate`, or `strong`, based on p-value thresholds |
| `practical_relevance` | `negligible`, `small`, `medium`, or `large`, based on effect size |
| `warnings` | Data-quality cautions such as low expected counts or imbalanced groups |
