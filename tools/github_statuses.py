...
def render_pr_comment(statuses):
    ...
    lines = [
        "## 🤖 Jenkins CI",
        "",
        "_Automated report generated by Jenkins._",
        "",
        f"**Build:** {build_mode()}  ",
        f"**Commit:** `{short_sha}`",
    ]
    if build_url:
        lines.append(f"**Jenkins:** {markdown_link('open build', build_url)}")
    ...
    if dashboard_url:
        lines.extend(["", f"📊 {markdown_link('Open quality dashboard', dashboard_url)}"])
